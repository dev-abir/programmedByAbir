...
public class buttonEventHandler_OLD implements EventHandler<ActionEvent> {

	int buttonRowPosition;
	int buttonColumnPosition;
	String tic_tac_toe_board[][];
	
	buttonEventHandler_OLD(String board[][], int x, int y) {
	...
	public void handle(ActionEvent event) {
		...
		if(tic_tac_toe_board[buttonRowPosition][buttonColumnPosition].equals("O")) {
			alert.setHeaderText("Information");
			alert.setContentText("Place already occupied");
			alert.show();
		}
		
		if(tic_tac_toe_board[buttonRowPosition][buttonColumnPosition].equals("X")) {
			alert.setHeaderText("Information");
			alert.setContentText("Place already occupied");
			alert.show();
		}
		...
		else if(new_tic_tac_toe_OBJECT.see_whether_it_is_a_draw_match_or_not()) {
	...
	void getComputerInput(int playerInputRow, int playerInputColumn){
		...
		computer_turn_calculator_OBJECT=new computer_turn_calculator(tic_tac_toe_board, "O", "X", 1);
